...
try {
  const headHash = execFileSync('git', ['rev-parse', 'HEAD'], { cwd: GIT_CWD, encoding: 'utf8' }).trim();
  const logOut   = execFileSync('git', ['log', `${baseRef}..HEAD`, '--oneline'], { cwd: GIT_CWD, encoding: 'utf8' }).trim();
  console.log(`[GIT SANITY] HEAD=${headHash}`);
  console.log(`[GIT SANITY] commits in range=${logOut.split('\n').filter(Boolean).length}`);
} catch (e) {
  console.log(`[GIT SANITY ERROR] ${e.message} ${e.stderr}`);
}

// ─── Git helpers ──────────────────────────────────────────────────────────────
// Uses execFileSync (not execSync) so args are never passed through /bin/sh.
// This prevents % format specifiers being interpreted as shell commands.
function git(...args) {
  try {
    ...
    console.log(`[GIT ERROR] git ${args.join(' ')} => ${e.stderr || e.message}`);
    return '';
  }
}
...
function getCommits(base) {
  ...
  return log.split('\n').filter(Boolean).map(line => {
    ...
    return { hash: hash.slice(0, 8), date, email, message: msgParts.join('|') };
  });
}
...
function getDiffSnapshot(base) {
  const diff = git('diff', `${base}..HEAD`, '--', '.', ':(exclude)package-lock.json', ':(exclude)*.lock');
...
function loadDtMembers() {
  ...
  if (env.trim()) {
    const members = new Set(env.split(',').map(s => s.trim().toLowerCase()).filter(Boolean));
    console.log(`DT members loaded from DT_MEMBERS env (${members.size}): ${[...members].join(', ')}`);
    return members;
  }
  ...
  if (fs.existsSync(filePath)) {
    ...
    console.log(`DT members loaded from .dt-members file (${members.size}): ${[...members].join(', ')}`);
    return members;
  }
...
function analyse(base) {
  ...
  for (const a of authors) {
    ...
    const type  = dtMembers.size === 0 ? 'DT'
                : dtMembers.has(login) ? 'DT' : 'DEV';

    if (!authorMap[login]) {
      authorMap[login] = {
        login, name: a.name, email: a.email, author_type: type,
        commit_count: 0, lines_added: 0, lines_deleted: 0,
        shas: [], dates: [], messages: [], filesMap: {},
      };
    }

    const stat   = diffStats[a.hash] || { added: 0, deleted: 0, files: [] };
    const commit = commits.find(c =>
      a.hash.startsWith(c.hash) || c.hash.startsWith(a.hash.slice(0, 8))
    );
    ...
    for (const f of stat.files) {
      if (!authorMap[login].filesMap[f.file]) {
        authorMap[login].filesMap[f.file] = { file: f.file, dt_lines: 0, dev_lines: 0, total_lines: 0 };
      }
  ...
  const finalContributors = contributors.map(c => {
    ...
    const devLinesOnDtFiles = c.author_type === 'DEV'
      ? dtOverlap.reduce((s, f) => s + f.total_lines, 0) : 0;
    return { ...c, dt_files_touched: dtOverlap.map(f => f.file).join(','), dev_lines_on_dt_files: devLinesOnDtFiles };
  });
...
function buildMarkdown(result, base) {
  ...
  const dtPct        = totalAdded > 0 ? ((dtAdded / totalAdded) * 100).toFixed(1) : '0.0';

  const allFilesMap = {};
  for (const c of contributors) {
    for (const f of c.files_detail) {
      if (!allFilesMap[f.file]) allFilesMap[f.file] = { file: f.file, dt_lines: 0, total_lines: 0 };
      allFilesMap[f.file].dt_lines    += f.dt_lines    || 0;
      allFilesMap[f.file].total_lines += f.total_lines || 0;
    }
  }
  const allFiles   = Object.values(allFilesMap).sort((a, b) => b.total_lines - a.total_lines);
  const allCommits = contributors
    .flatMap(c => c.commit_shas.map((sha, i) => ({
      sha, date: c.commit_dates[i] || '', message: c.commit_messages[i] || '',
      author: c.author, type: c.author_type,
    })))
  ...
  if (allFiles.length > 0) {
    ...
    for (const f of allFiles) {
      const pct = f.total_lines > 0 ? ((f.dt_lines / f.total_lines) * 100).toFixed(1) : '0.0';
      lines.push(`| \`${f.file}\` | ${f.dt_lines} | ${f.total_lines} | ${pct}% |`);
    }
    lines.push('');
  }
  ...
  if (allCommits.length > 0) {
    ...
    for (const c of allCommits) {
      ...
      lines.push(`| \`${c.sha}\` | ${c.date} | ${c.author} | ${badge} | ${c.message} |`);
    }
    lines.push('');
  }

  const overwrites = contributors.filter(c => c.author_type === 'DEV' && c.dev_lines_on_dt_files > 0);
  if (overwrites.length > 0) {
    lines.push('### Overwrites on DT Work', '');
    lines.push('| Author | Lines on DT Files | Files |', '|---|---|---|');
    for (const c of overwrites) {
      lines.push(`| ${c.author} | ${c.dev_lines_on_dt_files} | \`${c.dt_files_touched.replace(/,/g, '`, `')}\` |`);
    }
    lines.push('');
  }
...
(function main() {
  ...
  console.log(`Lines: ${dtAdded}/${totAdded} DT (${totAdded > 0 ? ((dtAdded / totAdded) * 100).toFixed(1) : 0}%)`);
})();